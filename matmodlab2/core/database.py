...
class _DatabaseFile(object):
    coordx = np.array([-0.5, 0.5, 0.5, -0.5, -0.5, 0.5, 0.5, -0.5]) + 0.5
    coordy = np.array([-0.5, -0.5, 0.5, 0.5, -0.5, -0.5, 0.5, 0.5]) + 0.5
    coordz = np.array([-0.5, -0.5, -0.5, -0.5, 0.5, 0.5, 0.5, 0.5]) + 0.5
...
class DatabaseFileWriter(_DatabaseFile):
    ...
    def initialize(self, glo_var_names, elem_var_names):
        ...
        self.fh.createVariable("qa_records", "c", ("num_qa_rec", "four", "len_string"))
        ...
        for (i, name) in enumerate(elem_var_names):
            ...
            self.fh.createVariable(
                "vals_elem_var{0}eb1".format(i + 1),
                "f",
        ...
        for i in range(3):
            ...
            self.fh.createVariable(
                "vals_nod_var{0}".format(i + 1), "f", ("time_step", "num_nodes")
            )
    ...
    def read_db(self, blk_num=1, elem_num=1):
        ...
        for (i, time) in enumerate(times):
            ...
            for var in name_elem_var:
                var_num = emap[var] + 1
                name = "vals_elem_var{0}eb1".format(var_num)
