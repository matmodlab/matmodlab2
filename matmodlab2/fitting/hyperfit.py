...
def lstsq(A, b):
    ...
    A = np.asarray(A)
    b = np.asarray(b)
    return np.dot(np.dot(inv(np.dot(A.T, A)), A.T), b)
...
class PolynomialHyperFit:
    ij = ((1, 0), (0, 1), (2, 0), (1, 1), (0, 2), (3, 0), (2, 1), (1, 2), (0, 3))
    ...
    def fit(self, xy, type=UNIAXIAL):
        ...
        self.fiterr = np.sqrt(np.mean((xy[:, 1] - fi) ** 2))
    ...
    def eval(self, strain, x=None, fac=None):
        ...
        return np.dot(A, x).flatten()

    def pprint(self, x=None):
        if x is None:
            x = self.x
        y = [
            "C{0}{1}={2:.8f}".format(i, j, x[k])
    ...
    def gendata(self, x, filename="data.csv"):
        strain = np.linspace(-0.25, 3.0, 100)
        s = self.eval(strain, x)
        noise = np.random.normal(0, 0.03 * np.amax(s), 100)
    ...
    def bp_plot(self, xy, x=None, plot=None):
        ...
        xp = np.linspace(np.amin(xy[:, 0]), np.amax(xy[:, 0]))
        yp = self.eval(xp, x)
