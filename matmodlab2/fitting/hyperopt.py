...
class HyperelasticOptimizer:
    ...
    def _single_opt_p(self, xdata, f):
        ...
        if pcov is None:
            ...
            if len(ydata) > len(p0):
                s_sq = (np.asarray(func(popt, *args)) ** 2).sum()
        ...
        yp = f(xdata, *popt, **kw)
        err = np.sqrt(np.mean((yp - ydata) ** 2)) / np.abs(np.average(ydata))
    ...
    def mp_plot(self, overlay=None, filename=None, show=True):
        ...
        ee = np.linspace(self.strain.min(), self.strain.max(), 100)
    ...
    def bp_plot(self, strain=None, overlay=None, points=True, **kwargs):
        ...
        if points:
            plot.circle(self.strain, self.stress, legend="{0}, data".format(self.dtype))
        if strain is None:
            strain = np.linspace(self.strain.min(), self.strain.max(), 100)
        ...
        if overlay is not None:
            ...
            for fit in overlay:
                ...
                plot.line(strain, ss, color="red", legend="{0}, fit".format(fit.dtype))
        return plot

    def todict(self):
        p = dict(
            [("C{0}{1}".format(i, j), self.popt[k]) for k, (i, j) in enumerate(self.IJ)]
        )
        return p

    def summary(self):
        p = [
            "C{0}{1}={2:.3f}".format(i, j, self.popt[k])
...
def _hyperelastic(xdata, *p, **kw):
    ...
    # helper quantities
    nominal_stress = np.zeros_like(xdata)
    xdata = np.asarray(xdata)
    I = np.ones(3)
    for (ix, x) in enumerate(xdata):
        I1 = np.sum(x)
        I2 = (I1**2 - np.sum(x * x)) / 2.0
        xi = 1.0 / x
        ...
        B = np.zeros((2, 3))
        B[0] = I - I1 * xi / 3.0
        B[1] = I1 * I - xi - 2.0 * I2 * xi / 3.0

        pk2_stress = np.sum(A[j] * B[j] for j in [0, 1])

        # Nominal stress
        nominal_stress[ix] = np.sqrt(x) * pk2_stress
...
def _data_type_helpers(dtype, strain):
    ...
    stretch = np.asarray(strain) + 1
    if dtype == UNIAXIAL_DATA:
        C = np.array([[lam, 1.0 / np.sqrt(lam), 1.0 / np.sqrt(lam)] for lam in stretch])
        return C, _uniaxial_func
    elif dtype == BIAXIAL_DATA:
        C = np.array([[lam, lam, 1.0 / lam**2] for lam in stretch])
        return C, _biaxial_func
    elif dtype == SHEAR_DATA:
        C = np.array([[lam, 1.0 / lam, 1.0] for lam in stretch])
...
def hyperopt(dtype, strain, stress, order=None, i2dep=None):
    ...
    if i2dep is None:
        ...
        nn = len(IJ(order, i2dep=i2dep))
        if nn > strain.shape[0]:
            raise OptimizeError("Order of fit too high for data")
...
def hyperopt2(*args, **kwargs):
    ...
    fopt.error2 = error / np.average(np.abs(fopt.stress))
    fopt.dtype2 = "Multi"
    return fopt
