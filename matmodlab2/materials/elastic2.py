from numpy import dot, zeros, triu
...
class AnisotropicElasticMaterial(Material):
    ...
    def __init__(self, **parameters):
        ...
        # in parameter array
        param_names = [
            "C{0}{1}".format(i + 1, j + 1) for i in range(6) for j in range(i, 6)
        ]

        # Create the parameter array and check inputs
        self.params = zeros(len(param_names))
        for (i, name) in enumerate(param_names):
            self.params[i] = parameters.pop(name, 0.0)
        if parameters:
            unused = ", ".join(list(parameters.keys()))
            logger.warning("The following parameters were " "unused {0}".format(unused))
