...
class PlasticMaterial(Material):
    ...
    **kwds : dict
    ...
    def eval(self, time, dtime, temp, dtemp, F0, F, strain, d, stress, statev, **kwds):
        ...
        i1, rootj2, _ = invariants(stress, "mechanics")
        if rootj2 - (A1 - A4 * i1) <= 0.0:
            ...
            N = np.sqrt(2.0) * A4 * I6 + s / magnitude(s)
            ...
            i1_p, rootj2_p, _ = invariants(P, "mechanics")
            if A4 != 0.0 and i1 > A1 / A4 and rootj2 / (i1 - A1 / A4) < rootj2_p / i1_p:
    ...
    def dot_with_elastic_stiffness(self, A):
        return 3.0 * self.params["K"] * isotropic_part(A) + 2.0 * self.params[
            "G"
        ] * deviatoric_part(A)
