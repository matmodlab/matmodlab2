...
class TrescaMaterial(Material):
    ...
    **kwds : dict
    ...
    def yield_function(self, stress):
        ...
        return r / np.sqrt(2.0) * np.cos(theta) - self.params["A1"]
    ...
    def yield_function_gradient(self, stress):
        ...
        term1 = np.cos(theta) / (np.sqrt(2.0) * r) * S
        ...
        coef2 = r / np.sqrt(2.0) * np.sin(theta) / (3.0 * np.sqrt(1.0 - lode**2))
        deriv = lode * (S2 / j3 - 3.0 / (r**2) * S)
    ...
    def simple_return(self, stress):
        ...
        if r * np.sin(abs(theta)) > np.sqrt(2.0 / 3.0) * self.params["A1"]:
            # Vertex
            r_scaled = np.sqrt(2.0 / 3.0) * self.params["A1"] / np.sin(abs(theta))
    ...
    def ddsdde(self):
        ...
        K3 = 3.0 * self.params["K"]
        G2 = 2.0 * self.params["G"]
        Lam = (K3 - G2) / 3.0
        # elastic stiffness
        ddsdde = np.zeros((6, 6))
    ...
    def eval(self, time, dtime, temp, dtemp, F0, F, strain, d, stress, statev, **kwds):
        ...
        P += 3.0 * self.params["K"] * isotropic_part(deps)
        S += 2.0 * self.params["G"] * deviatoric_part(deps)
