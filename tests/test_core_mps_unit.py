...
def test_mps_assign_material():
    ...
    mps.material = ElasticMaterial(E=10, Nu=0.1)
...
def test_mps_db_exo():
    ...
    mps1.material = ElasticMaterial(E=10, Nu=0.1)
    mps1.run_step("E", 1)
...
def test_mps_run_step_uni_strain():
    ...
    mps = MaterialPointSimulator(jobid)
    mps.material = ElasticMaterial(E=10, Nu=0.1)
    s = mps.run_step("EEE", [1, 0, 0], scale=0.1)
    assert isclose(s.components[0], 0.1)
...
def test_mps_run_step_pres():
    ...
    mps = MaterialPointSimulator(jobid)
    mps.material = ElasticMaterial(E=10, Nu=0.1)
    s = mps.run_step("SSS", [1, 1, 1], scale=1.0)
    assert isclose(np.sum(s.components[:3]), 3)
    s = mps.run_step("SSS", [1, 1, 1], scale=2.0)
...
def test_mps_run_step_ivalid_F_1():
    ...
    s = mps.run_step("SSS", [1, 1, 1], scale=1.0)
    try:
        s = mps.run_step("F", [1, 1, 1], scale=2.0)
...
def test_mps_run_step_invalid_F_3():
    ...
    mps.material = ElasticMaterial(E=10, Nu=0.1)
    try:
        s = mps.run_step("F", [1.05, 0.05, 0, 0.05, 1, 0, 0, 0, 1], kappa=0)
    except ValueError as e:
        assert (
            e.args[0]
            == "QR decomposition of deformation gradient gave unexpected rotations (rotations are not yet supported)"
        )
