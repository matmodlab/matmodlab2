...
def test_isotropic_part():
    a_ident = np.array([1.0, 1.0, 1.0, 0.0, 0.0, 0.0])
    A_ident = np.eye(3)
...
def test_deviatoric_part():
    a_dev = np.array([1.0, -0.5, -0.5, 0.0, 0.0, 0.0])
    A_dev = np.array([[1.0, 0.0, 0.0], [0.0, -0.5, 0.0], [0.0, 0.0, -0.5]])
...
def test_invariants_mechanics_dev():
    a_dev = np.array([1.0, 0.0, -1.0, 2.0, 4.0, 3.0])
    A_dev = np.array([[1.0, 2.0, 3.0], [2.0, 0.0, 4.0], [3.0, 4.0, -1.0]])
    a_i1 = 0.0
    a_rootj2 = np.sqrt(30)
    print(tens.magnitude(a_dev), np.sqrt(60.0))
    print(tens.magnitude(A_dev), np.sqrt(60.0))
    # Do it with 6x1
...
def test_invariants_mechanics_iso():
    a_iso = np.array([4.5, 4.5, 4.5, 0.0, 0.0, 0.0])
    A_iso = np.eye(3) * 4.5
    a_i1 = 3 * 4.5
    # Do it with 6x1
    i1, rootj2, j3 = tens.invariants(a_iso, "mechanics")
    assert np.isclose(a_i1, i1)
    assert np.isclose(0.0, rootj2)
    assert np.isclose(0.0, j3)
    # Now do it with 3x3
    i1, rootj2, j3 = tens.invariants(A_iso, "mechanics")
    assert np.isclose(a_i1, i1)
    assert np.isclose(0.0, rootj2)
...
def test_invariants_lode():
    def f(x):
        fac = -1.0 + 2.0 * x
        return np.array([[1.0, 0.0, 0.0], [0.0, fac, 0.0], [0.0, 0.0, -1.0]])

    z, r, theta, lode = tens.invariants(f(0.5), "lode")
    assert np.isclose(0.0, theta)
    z, r, theta, lode = tens.invariants(f(1.0), "lode")
    assert np.isclose(-np.pi / 6.0, theta)
    z, r, theta, lode = tens.invariants(f(0.0), "lode")
    assert np.isclose(np.pi / 6.0, theta)
...
def test_magnitude():
    a_ident = np.array([1.0, 1.0, 1.0, 0.0, 0.0, 0.0])
    A_ident = np.eye(3)
    assert isclose(tens.magnitude(a_ident), np.sqrt(3.0))
    assert isclose(tens.magnitude(A_ident), np.sqrt(3.0))

    mag = np.sqrt(1 + 2.0 * 0.5**2)
    a_dev = np.array([1.0, -0.5, -0.5, 0.0, 0.0, 0.0])
    A_dev = np.array([[1.0, 0.0, 0.0], [0.0, -0.5, 0.0], [0.0, 0.0, -0.5]])
...
def test_trace():
    a_ident = np.array([1.0, 1.0, 1.0, 0.0, 0.0, 0.0])
    A_ident = np.eye(3)
    assert isclose(tens.trace(a_ident), 3.0)
    assert isclose(tens.trace(A_ident), 3.0)

    a_dev = np.array([1.0, -0.5, -0.5, 0.0, 0.0, 0.0])
    A_dev = np.array([[1.0, 0.0, 0.0], [0.0, -0.5, 0.0], [0.0, 0.0, -0.5]])
    assert isclose(tens.trace(a_dev), 0.0)
    assert isclose(tens.trace(A_dev), 0.0)
