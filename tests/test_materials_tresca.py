...
def f(x):
    fac = -1.0 + 2.0 * x
    A = np.array([[1.0, 0.0, 0.0], [0.0, fac, 0.0], [0.0, 0.0, -1.0]])
    return A


def test_tresca_yield_function_1():
    tresca = TrescaMaterial(K=1, G=1, A1=1.0)
    assert np.isclose(-1.0, tresca.yield_function(np.zeros(6)))
    assert np.isclose(-1.0, tresca.yield_function(np.zeros((3, 3))))
...
def test_tresca_radial_return_txe():
    ...
    assert np.allclose(2.0 / 3.0 * A, sig_ret)
    assert np.isclose(0.0, tresca.yield_function(sig_ret))
...
def test_tresca_simple_return_dev():
    A = np.array([[1, 0, 0], [0, 0, 0], [0, 0, -1]])
    tresca = TrescaMaterial(K=1, G=1, A1=1.0)
    sig_ret = tresca.simple_return(2.0 * A)
    assert np.allclose(A, sig_ret)
    assert np.isclose(0.0, tresca.yield_function(sig_ret))
...
def test_tresca_eval():
    tresca = TrescaMaterial(K=8.0, G=3.0, A1=1.0)
    A = np.array([[1, 0, 0], [0, 0, 0], [0, 0, -1]])
    I3, Z3x3 = np.eye(3), np.zeros((3, 3))
    s, x, C = tresca.eval(1.0, 1.0, 0.0, 0.0, I3, I3, Z3x3, Z3x3, A, None)
...
def test_tresca_multi_step_verification():
    ...
    for idx in range(0, len(table_data) - 1):
        ...
        for jdx in range(N):
            # Update the stress
            stress, _, _ = tresca.eval(t0, dt, 0.0, 0.0, I3, I3, Z3x3, d, stress, None)
